...
def _placement(polygon, sheet_index=0):
    return Placement(part_name="p", sheet_index=sheet_index, position=(0., 0.),
                      rotation_degrees=0., mirrored=False, polygon=polygon)


def test_render_sheet_preview_png_bytes_produces_a_valid_png():
    placements = [_placement([(0., 0.), (1., 0.), (1., 1.), (0., 1.)])]
...
def test_save_sheet_preview_writes_a_png_file(tmp_path):
    ...
    save_sheet_preview(Sheet(width=3, height=2), placements, str(out_path),
                        sheet_number=1, utilization=0.75)
