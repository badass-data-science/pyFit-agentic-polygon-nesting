...
* Done, actually: an MCP interface. `sheetnest-mcp` exposes packing, preview, reporting, and file export as tools an agent can call directly, the same idea as pyLair's own MCP server (see "An Agentic Interface, Not Just a Command Line" above).
