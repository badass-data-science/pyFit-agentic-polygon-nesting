...
writes `output/nest_sheet1.dxf`, `output/nest_sheet2.dxf`, ... (one file per sheet actually used) plus `output/nest_report.json`, and prints the same report to stdout.
...
- **Rectangular sheets only.** No support (yet) for irregular stock outlines or offcut reuse.
- **No sheet-to-sheet part reuse across a job's remaining scrap.** Each sheet is packed independently; leftover space on sheet 1 isn't considered when starting sheet 2.
...
| `sheetnest/packer.py` | The bottom-left-fill placement heuristic. |
